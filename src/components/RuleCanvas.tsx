...
const nodeTypes = { camera: CameraNode, detector: DetectorNode, action: ActionNode, vlmSearch: VLMSearchNode };
...
export const RuleCanvas = () => {
    ...
    const onDrop = useCallback((event: React.DragEvent) => {
        ...
        const newNode: Node<CustomNodeData> = { id: uuidv4(), type: nodeType, position, data: data as CustomNodeData };
        addNode(newNode);
    }, [addNode]);

    return (
        <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect}
            nodeTypes={nodeTypes} onInit={inst => { reactFlowInstance.current = inst; }} onDragOver={onDragOver} onDrop={onDrop}
            fitView className="bg-transparent" deleteKeyCode={['Backspace', 'Delete']}
            defaultEdgeOptions={{ animated: true, style: { stroke: 'url(#edge-gradient)', strokeWidth: 1.2 } }}>
            <defs>
