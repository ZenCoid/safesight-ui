...
export const NodePalette = ({ cameras }: Props) => {
    ...
    return (
        <aside className="w-52 bg-[#0a0a0a] border-r border-machined p-4 flex flex-col gap-5 overflow-y-auto z-20">
            <h2 className="text-[10px] text-slate-600 uppercase tracking-[0.3em]">Nodes</h2>
            <div>
                <h3 className="text-[10px] text-slate-700 tracking-widest mb-2">CAMERAS</h3>
                {cameras.map(cam => (
                    <div key={cam.id} draggable onDragStart={e => onDragStart(e, 'camera', { cameraId: cam.id, label: cam.name })}
                        className="flex items-center gap-2 text-sm text-slate-500 hover:text-white transition-colors cursor-grab py-1">
                        <Camera className="w-4 h-4" /> {cam.name}
                    </div>
                ))}
            </div>
            <div>
                <h3 className="text-[10px] text-slate-700 tracking-widest mb-2">DETECTORS</h3>
                <div draggable onDragStart={e => onDragStart(e, 'detector', { modules: ['person', 'helmet'] })}
                    className="flex items-center gap-2 text-sm text-slate-500 hover:text-purple-400 cursor-grab py-1">
                ...
                <div draggable onDragStart={e => onDragStart(e, 'vlmSearch', { query: '', channel: 'whatsapp', intervalFrames: 10 })}
                    className="flex items-center gap-2 text-sm text-slate-500 hover:text-cyan-400 cursor-grab py-1">
                    <BrainCircuit className="w-4 h-4" /> VLM Search
                </div>
            </div>
            <div>
                <h3 className="text-[10px] text-slate-700 tracking-widest mb-2">ACTIONS</h3>
                <div draggable onDragStart={e => onDragStart(e, 'action', { channels: ['whatsapp'] })}
                    className="flex items-center gap-2 text-sm text-slate-500 hover:text-amber-400 cursor-grab py-1">
